...
def heave_peak_to_peak(series: pd.Series) -> float:
    s = pd.to_numeric(series, errors="coerce").dropna()
    if s.empty:
        return float("nan")
    centered = s - s.mean()
    return float(centered.max() - centered.min())
...
def motion_diagnostics(df: pd.DataFrame, low_hz: float = 0.03, high_hz: float = 0.35) -> pd.DataFrame:
    ...
    for col, unit in mapping:
        ...
        row = {
            ...
            "sample_dt_s": dt,
            "max_heave_amplitude": np.nan,
            "heave_peak_to_peak": np.nan,
            "heave_accel_rms_m_s2": np.nan,
        }

        if col == "heave_m":
            row["max_heave_amplitude"] = max_heave_amplitude(raw)
            row["heave_peak_to_peak"] = heave_peak_to_peak(raw)
            row["heave_accel_rms_m_s2"] = heave_acceleration_rms(
...
def build_zip(results: dict[str, dict]) -> bytes:
    ...
    with zipfile.ZipFile(mem, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        for name, payload in results.items():
            ...
            if payload.get("diagnostics_df") is not None:
...
with st.sidebar:
    ...
    if mode == "Target heading ± tolerance":
        target_heading = st.number_input(
            "Target heading (deg)", value=170.0, min_value=0.0, max_value=360.0, step=0.1
        )
        tolerance = st.number_input(
            "Tolerance (deg)", value=5.0, min_value=0.0, max_value=180.0, step=0.1
        )
    elif mode == "Heading range":
        min_heading = st.number_input(
            "Min heading (deg)", value=165.0, min_value=0.0, max_value=360.0, step=0.1
        )
        max_heading = st.number_input(
            "Max heading (deg)", value=175.0, min_value=0.0, max_value=360.0, step=0.1
        )
    ...
    if results:
        ...
        if payload["diagnostics_df"] is not None:
            ...
            if not heave_diag.empty:
                ...
                h2.metric("Heave peak-to-peak", f"{heave_diag['heave_peak_to_peak'].iloc[0]:.3f} m")
                h3.metric(
                    "RMS heave acceleration",
                    f"{heave_diag['heave_accel_rms_m_s2'].iloc[0]:.3f} m/s²",
                )
